...
drinking_2012 <- select(any_drinking, "state", "location", "both_sexes_2012", "females_2012", "males_2012")

# Using the (new) 2012 data, create a column `diff` that has 
# the difference in male and female drinking rates
drinking_2012 <- mutate(drinking_2012, diff = abs(females_2012 - males_2012))
...
write.csv(drinking_2012, "output/diff_2012.csv", row.names = FALSE)
...
diff_f_m <- any_drinking$females_2012 - any_drinking$males_2012
more_f_than_m <- drinking_2012 %>%
  mutate(f_larger_than_m = diff_f_m) %>%
  filter(f_larger_than_m > 0) %>%
  select("state", "location", "f_larger_than_m")
write.csv(more_f_than_m, "output/more_f_than_m.csv", row.names = FALSE)
...
most_similar <- filter(drinking_2012, diff == min(diff)) %>%
  select("state", "location", "diff")
write.csv(most_similar, "output/most_similar.csv", row.names = FALSE)
...
state_only <- drinking_2012 %>%
  filter(state == location) %>%
  select(-location)
write.csv(state_only, "output/state_only.csv", row.names = FALSE)
...
highest_state <- filter(state_only, both_sexes_2012 == max(both_sexes_2012)) %>%
  select("state", "both_sexes_2012")
write.csv(highest_state, "output/highest_state.csv", row.names = FALSE)
# Which state had the **lowest** drinking rate for both sexes combined?
# Your answer should be a *dataframe* of the state and value of interest
# Write this data frame to `lowest_state.csv`.
lowest_state <- filter(state_only, both_sexes_2012 == min(both_sexes_2012)) %>%
  select("state", "both_sexes_2012")
write.csv(lowest_state, "output/lowest_state.csv", row.names = FALSE)
...
get_state_data <- function(state_name){
  state_data <- any_drinking %>%
    filter(state == state_name)
  write.csv(state_data, paste0("output/", state_name ,"_data.csv"), row.names = FALSE)
}
...
average_both_sexes <- mean(country_only$both_sexes_2012)
answer[["avg_both_sexes"]] <- average_both_sexes 
...
largest_increase_name <- country_only %>%
  mutate(increase = males_2012 - males_2002) %>%
  filter(increase == max(increase))
answer[["largest_male_increase"]] <- largest_increase_name$location
...
male_increase <- country_only %>%
  filter(males_2012 - males_2002 > 0)
num_male_increase <- nrow(male_increase)
answer[["num_male_increase"]] <- num_male_increase
...
answer[["frac_male_increase"]] <- num_male_increase / nrow(country_only)
...
female_increase <- country_only %>%
  filter(females_2012 - females_2002 > 0)
num_female_increase <- nrow(female_increase)
answer[["num_female_increase"]] <- num_female_increase
...
answer[["frac_female_increase"]] <- num_female_increase / nrow(country_only)
...
f_increase_m_decrease <- country_only %>%
  filter(females_2012 - females_2002 > 0, males_2012 - males_2002 < 0)
num_f_increase_m_decrease <- nrow(f_increase_m_decrease)
answer[["num_f_increase_m_decrease"]] <- num_f_increase_m_decrease

# Convert your list to a data frame, and write the results 
# to the file `binge_info.csv`
binge <- data.frame(answer)
write.csv(binge, "output/binge_info.csv", row.names = FALSE)
...
max_binge <- binge_drinking %>%
  group_by(state) %>%
  filter(both_sexes_2012 == max(both_sexes_2012), state != "National") %>%
  select("state", "location", "both_sexes_2012")
write.csv(max_binge, "output/max_binge.csv", row.names = FALSE)
...
names(any_drinking)[ which( names(any_drinking) == "any_state" ) ] <- "state"
names(any_drinking)[ which( names(any_drinking) == "any_location" ) ] <- "location"
...
names(binge_drinking)[ which( names(binge_drinking) == "binge_state" ) ] <- "state"
names(binge_drinking)[ which( names(binge_drinking) == "binge_location" ) ] <- "location"
...
mixed_data <- left_join(any_drinking, binge_drinking, by = c("state", "location"))
# Create a column `diff_2012` storing the difference between `any` and `binge` 
# drinking for both sexes in 2012
mixed_data <- mutate(mixed_data, diff_2012 = abs(any_both_sexes_2012 - binge_both_sexes_2012))
...
biggest_abs_diff_2012 <- mixed_data %>%
  filter(diff_2012 == max(diff_2012)) %>%
  select("state", "location", "diff_2012")
write.csv(biggest_abs_diff_2012, "output/biggest_abs_diff_2012.csv", row.names = FALSE)

# Which location has the smallest *absolute* difference between `any` and 
# `binge` drinking? Your answer should be a one row data frame with the state, 
# location, and column of interest (diff_2012). 
# Write this dataframe to `smallest_abs_diff_2012.csv`.
smallest_abs_diff_2012 <- mixed_data %>%
  filter(diff_2012 == min(diff_2012)) %>%
  select("state", "location", "diff_2012")
write.csv(smallest_abs_diff_2012, "output/smallest_abs_diff_2012.csv", row.names = FALSE)
...
highest_drinking <- function(year){
  both_sexes_year <- paste0("any_both_sexes_", year)
  any_drinking_year <- select(mixed_data, "state", "location", both_sexes_year)
  highest_drinking <- any_drinking_year[any_drinking_year[, 3] == max(any_drinking_year[, 3]),]
}

highest_2002 <- highest_drinking(2002)
################################### Challenge ##################################
...
sapply(state_only$state, get_state_data)
...
state_year_filter <- function(dataframe, year, state_name){
  if(isTRUE(all_equal(dataframe, any_drinking))){
    sexes_year <- paste0("any_both_sexes_", year)
    state_year_dtf <- dataframe %>%
      filter(state == state_name) %>%
      select_("state", "location", "sexes_year")
    name <- "any"
  }
  else{
    sexes_year <- paste0("binge_both_sexes_", year)
    state_year_dtf <- dataframe %>%
      filter(state == state_name) %>%
      select_("state", "location", "sexes_year")
    name <- "binge"
  }
  write.csv(state_year_dtf, paste0("output/", name, "_", state_name, "_", year, ".csv"), row.names = FALSE)
}

# Create the file `binge_Colorado_2007.csv` using your function.
state_year_filter(binge_drinking, 2007, "Colorado")
